...
# Pull out normalized expression for later use
normalized_sc_matrix <- my_scrna_object@norm_expr

# Get cell type vector
cell_type_vector <- pDataDT(my_scrna_object)$merge_cell_types

# Find differentially expressed genes and create sign_list
scran_markers_subclusters_sc <- findMarkers_one_vs_all(gobject = my_scrna_object,
                                                       method = 'scran',
                                                       expression_values = 'normalized',
                                                       cluster_column = 'merge_cell_types')

Sig_scran <- scran_markers_subclusters_sc[which(scran_markers_subclusters_sc$ranking <= 100),]
sign_list <- list()
for(cell_name in c('DC','Endothelial','Epithelial','Fibroblast','LC','Macrophage','Mature DC','Melanocyte','pDC','T Cell')) {
  sign_list[[cell_name]] <- Sig_scran$genes[which(Sig_scran$cluster == cell_name)]
}
```
...
scran_markers_subclusters <- findMarkers_one_vs_all(gobject = my_giotto_object,
                                                    method = 'scran',
                                                    expression_values = 'normalized',
                                                    cluster_column = 'leiden_clus')
...
```{r}
# list of signature genes from scRNAseq analysis
sign_list <- sign_list
...
cellProximityNetwork(gobject = my_giotto_object,
                     ...
                     save_param = list(save_name = 'fig_24_cellProximityNetwork', 
                                       ...
                                       base_height = 6, 
                                       save_format = 'pdf'))
# cellProximityNetwork may plot the nodes differently every time, but interactions are the same.
```
...
spatPlot3D(gobject = my_giotto_object_3D,
           axis_scale = "cube",
           return_plot = FALSE,
           save_param = list(save_name = 'fig_36a_spatPlots_3D'))
spatPlot3D(gobject = my_giotto_object_3D,
           axis_scale = "real",
           return_plot = FALSE,
           save_param = list(save_name = 'fig_36b_spatPlots_3D'))
```
...
spatPlot3D(gobject = my_giotto_object_3D,
           ...
           point_size = 5,
           cell_color_code = my_colors,
           save_param = list(save_name = 'fig_37a_leiden3D'),
           cell_color = "leiden_clus")
...
spatPlot2D(gobject = my_giotto_object_3D,
           ...
           coord_fix_ratio = 1,
           cell_color_code = my_colors,
           save_param = list(save_name = 'fig_37b_leiden3D', base_width = 6, base_height = 6, save_format = 'pdf'),
           cell_color = "leiden_clus")
```
